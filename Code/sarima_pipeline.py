...
CONFIG_OVERRIDES: dict[str, dict] = {
    ...
    # Uncomment to treat them as persistent bounded instead:
...
def plot_forecast(series: pd.Series, result: dict, col_name: str,
    ...
    ax.text(1.0, 1.02, metrics_label, transform=ax.transAxes,
...
def print_config_summary(df: pd.DataFrame, m: int = 4):
    ...
    for col in df.columns:
        ...
        ovr   = ' (ovr)' if col in CONFIG_OVERRIDES else ''
        print(f"  {col:<30s} {regime+ovr:<20s} {d_str:>3s} {D_str:>3s} {seas:>5s} {cfg['max_p']:>5d}   {tx}")
    print()
